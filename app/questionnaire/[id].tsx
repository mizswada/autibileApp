...
export default function QuestionnaireForm() {
      ...
      if (storedData) {
        ...
        if (data.selectedChildId) {
          ...
          const selectedChildData = {
            ...
            name: data.patientIds?.find((c: any) => c.patient_id === data.selectedChildId)?.fullname || 'Selected Child',
            age: null
          };
          
          setSelectedChild(selectedChildData);
          setChildren(data.patientIds?.map((c: any) => ({
   ...
   useEffect(() => {
       ...
       const fetchQuestionnaireData = async () => {
         try {
           ...
           const response = await API('apps/questionnaire/mobile', { 
             questionnaireID: id,
           ...
           if (response.statusCode === 200 && response.data) {
             ...
             if (questionnaireData.mchatr_eligibility) {
               ...
               if (id === '1') {
                 ...
                 const isDisabled = eligibility.mchatr_status === 'Disable' || 
  ...
  const fetchMainQuestions = async () => {
      try {
        ...
        if (questionnaire && questionnaire.questions) {
          const processedQuestions = questionnaire.questions.map((question: any) => {
  ...
  const handleConditionalLogic = (parentQuestionId: number, selectedOptionValue: any) => {
    try {
      // Find the current question
      const currentQuestion = questions.find(q => q.question_id === parentQuestionId);
      if (!currentQuestion) {
        return;
      }
      
      // Find the selected option in conditional_logic
      const conditionalLogic = currentQuestion.conditional_logic?.find((logic: any) => 
        logic.option_id === selectedOptionValue || logic.option_value === selectedOptionValue
      );
    ...
    let question = questions.find(q => q.question_id === parseInt(questionId));
    ...
    if (question) {
      ...
      for (const mainQ of questions) {
        if (mainQ.sub_questions && mainQ.sub_questions.length > 0) {
          const subQ = mainQ.sub_questions.find((sq: any) => sq.question_id === parseInt(questionId));
          if (subQ) {
            question = subQ;
    ...
    const isMainQuestion = questions.some(q => q.question_id === parseInt(questionId));
  ...
  const getQuestionType = (question: any) => {
    ...
    if (question.options && question.options.length > 0) {
      ...
      if (firstOption.option_title && firstOption.option_title.startsWith('[text]')) {
        return 'text';
      } else if (firstOption.option_title && firstOption.option_title.startsWith('[textarea]')) {
        return 'textarea';
      } else if (firstOption.option_title && firstOption.option_title.startsWith('[checkbox]')) {
        return 'checkbox';
      } else if (firstOption.option_title && firstOption.option_title.startsWith('[scale]')) {
        return 'scale';
      }
    }
  ...
  const hasConditionalLogic = (question: any) => {
    const hasLogic = question.conditional_logic && question.conditional_logic.length > 0;
    return hasLogic;
  };
  ...
  const calculateScore = (answers: { [questionId: string]: any }) => {
    ...
    const yesCount = Object.values(answers).filter((a) => a === 'yes' || a === 1).length;
        ...
        if (questionType === 'text' || questionType === 'textarea') {
          ...
          if (questionType === 'checkbox' && Array.isArray(selectedOptionId)) {
            ...
            selectedOptionId.forEach((optionId: any) => {
              const selectedOption = q.options?.find((opt: any) => opt.option_id === optionId);
              formattedAnswers.push({
                question_id: q.question_id,
                parent_question_id: null, // Main question has no parent
                option_id: optionId,
            ...
            const selectedOption = q.options?.find((opt: any) => opt.option_id === selectedOptionId);
            formattedAnswers.push({
              question_id: q.question_id,
              parent_question_id: null, // Main question has no parent
        ...
        if (q.sub_questions && q.sub_questions.length > 0) {
          q.sub_questions.forEach((subQ: any) => {
            ...
            if (subQuestionType === 'text' || subQuestionType === 'textarea') {
              ...
              if (subQuestionType === 'checkbox' && Array.isArray(selectedOptionId)) {
                // Handle multiple checkbox selections for sub-questions
                selectedOptionId.forEach((optionId: any) => {
                  const selectedOption = subQ.options?.find((opt: any) => opt.option_id === optionId);
                  formattedAnswers.push({
                    question_id: subQ.question_id,
                    parent_question_id: q.question_id, // Link to parent question
                    option_id: optionId,
                ...
                const selectedOption = subQ.options?.find((opt: any) => opt.option_id === selectedOptionId);
                formattedAnswers.push({
                  question_id: subQ.question_id,
                  parent_question_id: q.question_id, // Link to parent question
                  option_id: selectedOptionId || null,
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          if (apiResponse.statusCode === 200) {
            ...
            throw new Error(apiResponse.message || 'Failed to submit questionnaire');
          }
    ...
    <View style={styles.mainContainer}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {questions && questions.length ? (
                        ...
                        <View style={styles.optionsRow}>
                                ...
                                style={[
                                  ...
                                  (questionType === 'checkbox' 
                                    ? (Array.isArray(answers[q.question_id]) ? answers[q.question_id].includes(option.option_id) : false)
                                    : answers[q.question_id] === option.option_id) && styles.optionSelected,
                                ]}
                                onPress={() => handleOptionSelect(q.question_id, option.option_id)}
                              >
                                <Text style={[
                                  styles.optionText,
                                  (questionType === 'checkbox' 
                                    ? (Array.isArray(answers[q.question_id]) ? answers[q.question_id].includes(option.option_id) : false)
                                    : answers[q.question_id] === option.option_id) && styles.optionSelectedText
                                ]}>
                      ...
                      <View style={styles.rangeOptions}>
                        {[1, 2, 3, 4, 5].map(value => (
                          <TouchableOpacity
                            ...
                            style={[
                              styles.rangeButton,
                              answers[q.question_id] === value && styles.rangeButtonSelected
                            ]}
                            onPress={() => handleOptionSelect(q.question_id, value)}
                          >
                            <Text style={[
                              styles.rangeButtonText,
                              answers[q.question_id] === value && styles.rangeButtonTextSelected
                            ]}>
                              {value}
                            </Text>
                          </TouchableOpacity>
                  ...
                  {questionType === 'text' && (
                    <View style={styles.textInputContainer}>
                      <TextInput
                        ...
                        onChangeText={(text) => handleTextInput(q.question_id, text)}
                ...
                {q.sub_questions && q.sub_questions.length > 0 && (
                  <View style={styles.subQuestionsContainer}>
                    {q.sub_questions.map((subQ: any, subIdx: number) => {
                      ...
                      return (
                        <View key={subQ.question_id} style={styles.subQuestionBlock}>
                          <View style={styles.subQuestionIndicator}>
                            <Ionicons name="chevron-forward" size={16} color="#4db5ff" />
                            <Text style={styles.subQuestionText}>Follow-up Question {subIdx + 1}</Text>
                          </View>
                          ...
                          {(subQuestionType === 'radio' || subQuestionType === 'checkbox') && (
                            <View style={styles.optionContainer}>
                              {subQ.options && subQ.options.length > 0 ? (
                                <View style={styles.optionsRow}>
                                  {subQ.options.map((option: any, optionIndex: number) => (
                                    <TouchableOpacity
                                      key={`${subQ.question_id}-${option.option_id || optionIndex}`}
                                      style={[
                                        styles.optionButton,
                                        (subQuestionType === 'checkbox' 
                                          ? (Array.isArray(answers[subQ.question_id]) ? answers[subQ.question_id].includes(option.option_id) : false)
                                          : answers[subQ.question_id] === option.option_id) && styles.optionSelected,
                                      ]}
                                      onPress={() => handleOptionSelect(subQ.question_id, option.option_id)}
                                      activeOpacity={0.7}
                                    >
                                      <Text style={[
                                        styles.optionText,
                                        (subQuestionType === 'checkbox' 
                                          ? (Array.isArray(answers[subQ.question_id]) ? answers[subQ.question_id].includes(option.option_id) : false)
                                          : answers[subQ.question_id] === option.option_id) && styles.optionSelectedText
                                      ]}>
                                        {cleanOptionTitle(option.option_title)}
                                      </Text>
                                    </TouchableOpacity>
                          ...
                          {subQuestionType === 'text' && (
                            <View style={styles.textInputContainer}>
                              <TextInput
                                ...
                                onChangeText={(text) => handleTextInput(subQ.question_id, text)}
                                placeholder="Enter your answer here"
                                multiline={false}
                              />
                            </View>
          ...
          <Text style={styles.noQuestions}>No questions for this questionnaire.</Text>
        )}
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>Please select which child this questionnaire is for:</Text>
              ...
              {children.map((child, index) => (
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    ...
                    try {
                      const eligibilityResponse = await API('apps/questionnaire/mobile', { 
                        questionnaireID: id, 
