...
export default function PractitionerReport() {
  ...
  const fetchDiaryReports = async () => {
    ...
    try {
      ...
      const response = await API('apps/diaryReport/listAll', {
  ...
  const groupedByPatient = diaryReports.reduce((acc, entry) => {
    const patientName = entry.patient?.fullname || `Patient ${entry.patient_id}`;
  ...
  const handleDatePress = (patientName: string, date: string, entries: DiaryReport[]) => {
  ...
  if (error) {
    return (
      <View style={styles.centered}>
        ...
        <TouchableOpacity onPress={fetchDiaryReports} style={styles.retryButton}>
            ...
            <View key={patientName} style={styles.patientCard}>
              ...
              {Object.keys(groupedByPatient[patientName]).map((date) => (
                <TouchableOpacity 
                  ...
                  onPress={() => handleDatePress(patientName, date, groupedByPatient[patientName][date])}
                >
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Diary Reports - {selectedDate}</Text>
                ...
                <View key={entry.diary_id} style={styles.modalEntryCard}>
                  <View style={styles.modalEntryHeader}>
                    ...
                    <Text style={styles.modalEntryTime}>
                      {new Date(entry.date || entry.created_at).toLocaleTimeString()}
                    </Text>
                  </View>
                  ...
                  {entry.patient && (
                    <View style={styles.modalPatientInfo}>
                      <Text style={styles.modalPatientDetail}>
                        Age: {entry.patient.dob ? new Date(entry.patient.dob).getFullYear() : 'N/A'}
                      </Text>
                      <Text style={styles.modalPatientDetail}>
...
const styles = StyleSheet.create({
  ...
  retryButton: { backgroundColor: '#4db5ff', padding: 10, borderRadius: 8 },
  retryButtonText: { color: '#fff', fontWeight: 'bold' },
  noDataText: { color: '#888', textAlign: 'center', marginTop: 40 },
  patientCard: {
    backgroundColor: '#fff',
    borderRadius: 10,
    padding: 15,
  ...
  patientName: { fontSize: 18, fontWeight: 'bold', marginBottom: 8, color: '#226699' },
  dateCard: {
  ...
  dateHeader: { fontSize: 16, fontWeight: 'bold', color: '#24A8FF' },
  modalOverlay: {
    flex: 1,
  ...
  modalScrollContent: {
    paddingBottom: 20, // Add some padding at the bottom for the last entry
  },
  modalEntryCard: {
